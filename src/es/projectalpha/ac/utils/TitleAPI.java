...
public class TitleAPI {
	 ...
	 */
	public TitleAPI(String title, String subtitle, int fadeInTime, int stayTime,
				 int fadeOutTime) {
		this.title = title;
		this.subtitle = subtitle;
	...
	public void updateTimes(Player player) {
		if (TitleAPI.packetTitle != null) {
			try {
				Object handle = getHandle(player);
				Object connection = playerConnection.get(handle);
				Object[] actions = TitleAPI.packetActions.getEnumConstants();
				Object packet = TitleAPI.packetTitle.getConstructor(
						new Class[]{TitleAPI.packetActions, chatBaseComponent,
			...
			try {
				...
				Object serialized = nmsChatSerializer.getConstructor(
						...
						.newInstance(titleColor +
								ChatColor.translateAlternateColorCodes('&',
										this.title));
				Object packet = TitleAPI.packetTitle
						.getConstructor(
								new Class[]{TitleAPI.packetActions,
