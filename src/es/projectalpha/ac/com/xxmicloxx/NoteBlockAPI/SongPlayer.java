...
public abstract class SongPlayer {
	...
	public void addPlayer(Player p){
		synchronized (this) {
			if (!playerList.contains(p.getName())) {
				...
				ArrayList<SongPlayer> songs = NBAPI.playingSongs.get(p.getName());
				if (songs == null) {
					songs = new ArrayList<SongPlayer>();
				}
				songs.add(this);
				NBAPI.playingSongs.put(p.getName(), songs);
			}
		}
	}
	...
	public void removePlayer(Player p){
		synchronized (this) {
			...
			if (NBAPI.playingSongs.get(p.getName()) == null) {
				return;
			}
			ArrayList<SongPlayer> songs = new ArrayList<SongPlayer>(NBAPI.playingSongs.get(p.getName()));
			songs.remove(this);
			NBAPI.playingSongs.put(p.getName(), songs);
